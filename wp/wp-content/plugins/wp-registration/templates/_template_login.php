...
if ( isset($_REQUEST['login'] ) && $_REQUEST['login'] == 'failed')
	_e('Invalid Username or Password! Login Failed!', 'nm-wpregistration');
elseif( isset($_REQUEST['login'] ) && $_REQUEST['login'] == 'empty')
	_e('Username or password is empty!', 'nm-wpregistration');
?>
</div>

<p id="nav">
<?php if ($this->get_option('_register_url') != '') {?>
<a rel="nofollow" href="<?php echo $this->get_option('_register_url') ?>"><?php _e('Register', 'nm-wpregisration') ?></a> | 	
<?php } ?>
<a href="javascript:lost_password();" title="<?php _e('Password Lost and Found', 'nm-wpregisration') ?>"><?php _e('Lost your password?', 'nm-wpregisration') ?></a>
</p>
